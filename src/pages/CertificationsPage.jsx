  ...
  return (
    <section className="py-24 px-[6vw] md:px-[8vw] lg:px-[10vw] font-sans">
      ...
      <div className="text-center mb-16">
        <h2 className="text-4xl font-bold text-theme">CERTIFICATIONS</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {dataWithImages.map((item, index) => (
          <div
            key={index}
            className="bg-blue-100/50 dark:bg-slate-800/50 border border-blue-600 dark:border-slate-700 rounded-2xl p-6 shadow-xl hover:-translate-y-2 hover:shadow-blue-500/30 transition duration-300"
          >
            <img
              src={item.img}
              alt={item.title}
            ...
            <h3 className="text-xl font-semibold text-theme mb-1">{item.title}</h3>
            <p className="text-blue-800 text-sm font-semibold">{item.issuer}</p>
            <p className="text-theme text-xs mb-4">{item.date}</p>

            <p className="text-theme text-sm mb-4 leading-relaxed">{item.details}</p>
            <Link
